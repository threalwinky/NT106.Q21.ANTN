...
class RoomService:
    ...
    async def send_bytes(self, websocket: WebSocket, message: bytes) -> None:
    ...
    async def join_room(
        ...
        async with self.state.state_lock:
            ...
            if self.count_remote_participants(room) >= 1:
                raise ValueError("Room already has an active remote connection. Netrix 1.0.3 only allows one host and one remote peer.")
