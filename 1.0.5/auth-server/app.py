...
app = FastAPI(title="Netrix Auth Server", version="1.0.5")

JWT_CONFIG = load_jwt_config()

if JWT_CONFIG.secret == "netrix-dev-secret":
    warnings.warn(
        "NETRIX_JWT_SECRET is using the default dev value. Set a strong secret in production.",
        stacklevel=1,
    )
...
def issue_token(conn, username: str) -> TokenResponse:
...
async def register(request: Request) -> TokenResponse:
    ...
    def _run() -> TokenResponse:
        with acquire() as conn:
            ...
            conn.commit()
            return issue_token(conn, payload.username)
...
async def login(request: Request) -> TokenResponse:
    ...
    def _run() -> TokenResponse:
        with acquire() as conn:
            user = find_user(conn, payload.username)
...
def logout(authorization: str | None = Header(default=None)) -> dict[str, str]:
    ...
    with acquire() as conn:
        with conn.cursor() as cursor:
            cursor.execute(
                "UPDATE sessions SET revoked = TRUE WHERE jti = %s AND username = %s",
                (claims["jti"], claims["sub"]),
            )
        conn.commit()
