...
## Stream màn hình ở bản 1.0.5

Ở bản `1.0.5`, phần stream màn hình không còn chỉ dựa vào JPEG từng frame nữa. Host vẫn gửi theo nhịp khoảng `30 FPS`, nhưng đường chính bây giờ là encode H.264 bằng OpenH264 thông qua package `H264Sharp`.
...
Toàn bộ phần server có thể chạy bằng `docker-compose.yml` trong thư mục `Netrix/1.0.5`.
...
LAN mode dùng cho trường hợp hai máy ở cùng mạng nội bộ. User vẫn phải đăng nhập trước, nhưng client không hỏi load balancer để chọn node.

Thay vào đó, user chọn **LAN** từ dropdown góc phải trên của auth card, rồi nhập IP của máy chạy server vào ô bên dưới. Từ một địa chỉ IP duy nhất, client tự suy ra hai URL cần thiết:

- Auth: `http://{ip}:8001`
- Main server WebSocket: `ws://{ip}:8000/ws`

Ví dụ server chạy trên máy `192.168.1.50`, user chỉ cần nhập `192.168.1.50`. Client không yêu cầu nhập thêm URL nào.

LAN mode vẫn dùng `room_id + room_password`, vẫn gửi JWT trong `create_room` / `join_room`, và vẫn kiểm tra LAN scope để tránh join nhầm từ mạng khác. Nếu client ở máy khác không kết nối được, thường là do firewall chưa mở port `8001` (auth) hoặc `8000` (main server).
...
Ở LAN mode, user chọn **LAN** trên auth card và nhập IP của máy server. Client tự suy ra auth URL là `http://{ip}:8001` và WebSocket URL là `ws://{ip}:8000/ws` — không cần nhập hai trường riêng.
...
| `POST /logout` | Thu hồi session theo JTI trong PostgreSQL |
| `GET /validate` | Kiểm tra JWT và session còn hiệu lực không |
...
User phải đăng nhập để lấy JWT. Khi đăng xuất, client gọi `POST /logout` trên auth server để thu hồi session theo JTI trong PostgreSQL — token không còn dùng được ngay cả khi chưa hết hạn. Khi tạo hoặc join room, client gửi token để main server validate. Room luôn có password, và password này cũng được dùng để tạo key mã hóa payload trong room.
...
Sau đó lấy IP LAN của máy server, ví dụ `192.168.1.50`. Trên mỗi client:

1. Mở `Netrix.exe`
2. Chọn **LAN** từ dropdown góc phải trên của auth card
3. Nhập IP server vào ô phía dưới dropdown: `192.168.1.50`
4. Đăng nhập hoặc đăng ký — client sẽ gọi `http://192.168.1.50:8001`

Host tạo room. Peer nhập room ID và room password để join. Client sẽ kết nối tới `ws://192.168.1.50:8000/ws`.

Nếu peer ở máy khác không vào được, kiểm tra firewall của máy server. Port cần mở tối thiểu là `8001` (auth) và `8000` (main server). PostgreSQL port `5433` không cần mở cho Netrix client.
...
Bản `1.0.5` vẫn là MVP theo hướng rõ kiến trúc trước. Một room hiện chỉ hỗ trợ `1 host + 1 remote peer`. Room state vẫn lưu trong memory, nên chưa có session migration giữa các node. Client trong repo hiện là C# WinForms dành cho Windows.
...
Netrix `1.0.5` đã có kiến trúc đủ rõ để chạy và trình bày: auth tách riêng, load balancer chọn node, main server xử lý realtime, client xử lý UI/capture/render/input, còn shared package giữ phần security dùng chung.
