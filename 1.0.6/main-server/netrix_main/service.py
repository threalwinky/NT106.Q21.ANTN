...
class RoomService:
    def __init__(self, state: ServerState) -> None:
        self.state = state
        self._transport_locks: dict[int, asyncio.Lock] = {}
    ...
    def validate_role(self, role: str) -> str:
        ...
        if normalized not in {"host", "controller"}:
            raise ValueError("role must be host or controller")
        return normalized
    ...
    def resolve_source_ip(self, connection: Any) -> str:
        ...
        if hasattr(connection, "get_extra_info"):
            peername = connection.get_extra_info("peername")
            if isinstance(peername, tuple) and peername:
                return str(peername[0]).strip()
    ...
    def _transport_lock(self, transport: Any) -> asyncio.Lock:
        transport_id = id(transport)
        lock = self._transport_locks.get(transport_id)
        if lock is None:
            lock = asyncio.Lock()
            self._transport_locks[transport_id] = lock
        return lock

    async def _send_tcp_packet(self, writer: Any, payload: bytes) -> None:
        if len(payload) <= 0:
            raise ValueError("TCP packet cannot be empty")
        writer.write(len(payload).to_bytes(4, byteorder="big", signed=True))
        writer.write(payload)
        await writer.drain()

    async def send_json(self, transport: Any, message: dict[str, Any]) -> None:
        payload = json.dumps(message, separators=(",", ":")).encode("utf-8")
        lock = self._transport_lock(transport)
        async with lock:
            if hasattr(transport, "send_text"):
                await transport.send_text(payload.decode("utf-8"))
                return
            await self._send_tcp_packet(transport, payload)

    async def send_bytes(self, transport: Any, message: bytes) -> None:
        lock = self._transport_lock(transport)
        async with lock:
            if hasattr(transport, "send_bytes"):
                await transport.send_bytes(message)
                return
            await self._send_tcp_packet(transport, message)

    async def send_error(self, transport: Any, detail: str) -> None:
        await self.send_json(transport, {"type": "error", "detail": detail})

    async def close_transport(self, transport: Any) -> None:
        with contextlib.suppress(Exception):
            ...
            if wait_closed is not None:
                await wait_closed()
        self._transport_locks.pop(id(transport), None)

    async def broadcast(self, room: Room, message: dict[str, Any], exclude_client_id: str | None = None) -> None:
        recipients = [
            participant
            for participant in room.participants.values()
            if participant.client_id != exclude_client_id and participant.realtime_ready
        ]
    ...
    async def broadcast_bytes(self, room: Room, message: bytes, exclude_client_id: str | None = None) -> None:
        recipients = [
            participant
            for participant in room.participants.values()
            if participant.client_id != exclude_client_id and participant.realtime_ready
        ...
        for participant in recipients:
            try:
                await self.send_bytes(participant.websocket, message)
            except Exception:  # noqa: BLE001
                dead_clients.append(participant.client_id)
    ...
    async def close_room(self, room: Room, reason: str) -> None:
        ...
        for participant in list(room.participants.values()):
            self.state.participants_by_id.pop(participant.client_id, None)
            await self.close_transport(participant.websocket)
    ...
    async def set_controller_permission(
        ...
        await self.send_json(
            participant.websocket,
            {
                ...
                "detail": "Host approved remote control." if approved else "Host denied controller access. You can keep watching, but input stays blocked.",
                "room_id": room.room_id,
                "target_client_id": participant.client_id,
            },
        )
    ...
    async def join_room(
        ...
        role = self.validate_role(str(message.get("role", "controller")))
        if role == "host":
        ...
        async with self.state.state_lock:
            ...
            if self.count_remote_participants(room) >= 1:
                raise ValueError("Room already has an active remote connection. Netrix 1.0.6 only allows one host and one controller.")
