...
namespace client.Streaming.Codecs;
...
internal sealed class H264VideoEncoderService : IDisposable
{
    ...
    public RemoteFrame? Encode(Bitmap sourceBitmap)
    {
        ...
        try
        {
            using var preparedBitmapOwner = PrepareBitmap(sourceBitmap);
            var preparedBitmap = preparedBitmapOwner ?? sourceBitmap;
    ...
    private static unsafe byte[] CopyBitmapBytesAsRgb(Bitmap bitmap)
    {
        ...
        try
        {
            ...
            var bytes = GC.AllocateUninitializedArray<byte>(targetRowBytes * bitmap.Height);
            fixed (byte* targetBase = bytes)
            {
                for (var row = 0; row < bitmap.Height; row++)
                {
                    var source = (byte*)bitmapData.Scan0 + (row * bitmapData.Stride);
                    var target = targetBase + (row * targetRowBytes);
                    for (var sourceColumn = 0; sourceColumn < sourceRowBytes; sourceColumn += 4)
                    {
                        target[0] = source[sourceColumn + 2];
                        target[1] = source[sourceColumn + 1];
                        target[2] = source[sourceColumn];
                        target += 3;
                    }
                }
            }
