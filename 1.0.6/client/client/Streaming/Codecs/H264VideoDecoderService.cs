...
internal sealed class H264VideoDecoderService : IDisposable
{
    ...
    private static unsafe Bitmap CreateBitmapFromBgr(byte[] bytes, int width, int height)
    {
        ...
        try
        {
            ...
            fixed (byte* sourceBase = bytes)
            {
                ...
                {
                    var source = sourceBase + (row * sourceRowBytes);
                    var target = (byte*)bitmapData.Scan0 + (row * bitmapData.Stride);
                    for (var targetColumn = 0; targetColumn < targetRowBytes; targetColumn += 4)
                    {
                        target[targetColumn] = source[0];
                        target[targetColumn + 1] = source[1];
                        target[targetColumn + 2] = source[2];
                        target[targetColumn + 3] = 255;
                        source += 3;
                    }
                }
            }
