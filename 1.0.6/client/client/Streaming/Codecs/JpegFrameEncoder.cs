...
internal sealed class JpegFrameEncoder
{
    ...
    public RemoteFrame Encode(Bitmap sourceBitmap)
    {
        ...
        var bitmapToEncode = resizedBitmap ?? sourceBitmap;
        using var stream = new MemoryStream();
        using var encoderParameters = new EncoderParameters(1);
        encoderParameters.Param[0] = new EncoderParameter(Encoder.Quality, _jpegQuality);
        bitmapToEncode.Save(stream, JpegCodec, encoderParameters);
        return new RemoteFrame(stream.ToArray(), bitmapToEncode.Width, bitmapToEncode.Height);
    }
