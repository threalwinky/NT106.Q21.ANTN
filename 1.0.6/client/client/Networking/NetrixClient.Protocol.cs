...
internal sealed partial class NetrixClient
{
    ...
    {
        FrameReceived?.Invoke(SecureFramePacketCodec.Decode(_roomSecurity, packet));
    }
    ...
    private void HandleServerMessage(string json)
    {
        ...
        switch (messageType)
        {
            ...
            case "error":
                ...
                break;
            case "room_created":
                var createdSession = new RoomSessionInfo(
                ...
                ConfigureRoomSecurityFromPendingPassword(createdSession.RoomId);
                CurrentSession = createdSession;
                RoomReady?.Invoke(CurrentSession);
                break;
            case "joined_room":
                var joinedSession = new RoomSessionInfo(
                    root.GetProperty("room_id").GetString() ?? string.Empty,
                    root.GetProperty("client_id").GetString() ?? string.Empty,
                    ParseRole(root.GetProperty("role").GetString()),
                    ParseMode(root.GetProperty("access_mode").GetString()),
                ...
                ConfigureRoomSecurityFromPendingPassword(joinedSession.RoomId);
            ...
            case "room_state":
                ...
                break;
            case "frame":
                var frameCodec = ParseFrameCodec(root);
                var payloadBase64 = root.TryGetProperty("payload_base64", out var payloadElement)
                    ? payloadElement.GetString()
                    : root.GetProperty("jpeg_base64").GetString();
                var bytes = Convert.FromBase64String(payloadBase64 ?? string.Empty);
            ...
            case "control_denied":
                if (CurrentSession is not null && root.GetProperty("target_client_id").GetString() == CurrentSession.ClientId)
                {
                    CurrentSession = CurrentSession with { CanSendControl = false };
                }
                ControllerPermissionChanged?.Invoke(
                    new ControllerPermissionInfo(
                        root.GetProperty("room_id").GetString() ?? string.Empty,
    ...
    private void HandleSecurePayload(JsonElement root)
    {
        ...
        var senderClientId = root.TryGetProperty("sender_client_id", out var senderClientIdElement)
            ? senderClientIdElement.GetString() ?? string.Empty
            : string.Empty;
        var senderDisplayName = root.TryGetProperty("sender_display_name", out var senderDisplayNameElement)
            ? senderDisplayNameElement.GetString() ?? string.Empty
            : string.Empty;
        ...
        {
            case "frame":
                var frameCodec = ParseFrameCodec(secureRoot);
                var payloadBase64 = secureRoot.TryGetProperty("payload_base64", out var payloadElement)
                    ? payloadElement.GetString()
                    : ReadString(secureRoot, "jpeg_base64");
                FrameReceived?.Invoke(
                    new RemoteFrame(
                        Convert.FromBase64String(payloadBase64 ?? string.Empty),
            ...
            case "file_complete":
                ...
                break;
            case "clipboard":
                ClipboardReceived?.Invoke(ReadString(secureRoot, "text"));
                break;
            case "process_request":
                ProcessRequestReceived?.Invoke(
                    new ProcessRequestInfo(
                        RequestId: ReadString(secureRoot, "request_id"),
                        RequesterClientId: senderClientId,
                        RequesterDisplayName: senderDisplayName));
                break;
            case "process_snapshot":
                ProcessSnapshotReceived?.Invoke(ParseProcessSnapshot(secureRoot));
                break;
        }
    }
}
