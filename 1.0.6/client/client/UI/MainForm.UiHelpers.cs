...
partial class MainForm
{
    ...
    private void UpdateAuthNetworkUi()
    {
        if (_lanServerIpField is not null)
        {
            _lanServerIpField.Visible = CurrentMode == AppMode.Lan;
            _lanServerIpField.Parent?.PerformLayout();
        }
    }

    private string ResolveAuthServerUrl()
    {
        return $"{ResolveLoadBalancerUrl().TrimEnd('/')}/auth";
    }

    private string ResolveLoadBalancerUrl()
    {
        if (CurrentMode == AppMode.Lan)
        {
            ...
            return $"http://{ip}:10310";
        }

        return NetrixEndpoints.LoadBalancer;
    }
