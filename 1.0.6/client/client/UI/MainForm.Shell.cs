...
partial class MainForm
{
    ...
    private void UpdateShellStateUi()
    {
        ...
        _showProcessesButton.Text = "Show Processes";
    ...
    private void RefreshSessionChrome()
    {
        ...
        var modeText = mode == AppMode.Internet ? "Internet" : "LAN";
        ...
        SetButtonInteractive(_showProcessesButton, hasSession && !_isSessionActionInProgress);
        ...
        {
            ParticipantRole.Host => "Hosting this room. Remote input is only accepted from connected controllers.",
            ParticipantRole.Controller when !session.CanSendControl => "Waiting for host approval. You can watch the stream, but remote input is blocked.",
            ParticipantRole.Controller when _remoteInputActive => "Remote input is active. Use Escape to leave full screen.",
            ParticipantRole.Controller => "Click inside the remote screen before sending mouse or keyboard input.",
            _ => "Session is active.",
        };
    }
    ...
    private void RefreshInteractiveStyles()
    {
        ...
        StyleButton(_showProcessesButton, palette, "toolbar");
