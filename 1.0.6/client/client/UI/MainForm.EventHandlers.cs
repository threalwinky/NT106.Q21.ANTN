...
partial class MainForm
{
    private void WireEvents()
    {
        ...
        _showProcessesButton.Click += async (_, _) =>
        {
            if (!IsButtonInteractive(_showProcessesButton))
            {
                return;
            }

            await ShowProcessesAsync();
        };
        ...
        _authNetworkModeComboBox.SelectedIndexChanged += (_, _) => UpdateModeUi();
        ...
        _remoteScreenBox.DragEnter += (_, e) => HandleRemoteDragEnter(e);
        _remoteScreenBox.DragDrop += async (_, e) => await HandleRemoteDragDropAsync(e);
        ...
        _netrixClient.FrameReceived += QueueRemoteFrame;
        ...
        _netrixClient.ProcessRequestReceived += request => HandleProcessRequest(request);
        _netrixClient.ProcessSnapshotReceived += snapshot => OnUiThread(() => ShowProcessSnapshot(snapshot));
        ...
        _netrixClient.ClipboardReceived += text => OnUiThread(() => ApplyRemoteClipboardText(text));
